...
if($_POST) {
    ...
    if(!isset($_POST["prenom"]) || !isset($_POST["userEmail"]) || !isset($_POST["userSubject"]) || !isset($_POST["userMessage"])) {
    ...
    $emailcontent = 'Hey! You have received a new message from the visitor <strong>'.$_POST["prenom"].'</strong><br/><br/>'. "\r\n" .
                'His message: <br/> <em>'.$_POST["userMessage"].'</em><br/><br/>'. "\r\n" .
                '<strong>Feel free to contact '.$_POST["prenom"].' via email at : '.$_POST["userEmail"].'</strong>' . "\r\n" ;
    ...
    if(!$Mailsending) {
        ...
        $output = json_encode(array('type'=>'message', 'text' => '<i class="icon ion-checkmark-round"></i> Hello '.$_POST["prenom"] .', Your message has been sent, we will get back to you asap !'));
        die($output);
    }
}
?>